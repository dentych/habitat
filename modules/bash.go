...
func (b bash) Install() {
	...
	bashAliases["cdgit"] = fmt.Sprintf("cd %s/Documents/git", main.HomeDir)

	for k, v := range bashAliases {
		output.WriteString(fmt.Sprintf("alias %s=\"%s\"\n", k, v))
	}

	output.WriteString(fmt.Sprintf("\nsource %s/.env/%s\n", main.HomeDir, gitPromptFileName))
	output.WriteString("PS1='\\[\\e[32m\\]\\u@\\h \\[\\e[33m\\]\\w\\[\\e[92m\\]$(__git_ps1 \" (%s)\")\\[\\e[00m\\] $ '\n")

	fmt.Println("Creating git prompt script file")
	err1 := ioutil.WriteFile(fmt.Sprintf("%s/.env/%s", main.HomeDir, gitPromptFileName), []byte(main.GitPrompt), 0644)
	fmt.Println("Creating bash setup script file")
	err2 := ioutil.WriteFile(fmt.Sprintf("%s/.env/%s", main.HomeDir, bashFileName), output.Bytes(), 0644)
	if err1 != nil || err2 != nil {
		log.Fatalln("Could not write files:", err1, err2)
	}

	fmt.Println("Adding bash setup script file to .bashrc file")
	main.AddFileToBashrc(fmt.Sprintf("%s/.env/%s", main.HomeDir, bashFileName))
	fmt.Println("Done!")
}

func (bash) Uninstall() {
	_ = os.Remove(fmt.Sprintf("%s/%s", main.HomeDir, gitPromptFileName))
	_ = os.Remove(fmt.Sprintf("%s/%s", main.HomeDir, bashFileName))
}
